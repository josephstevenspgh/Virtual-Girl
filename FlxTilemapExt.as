...
{	
	...
	{		
		...
		public static const DBLSLOPE_FLOOR_LEFT1:uint	= 4;
		public static const DBLSLOPE_FLOOR_LEFT2:uint	= 5;
		public static const DBLSLOPE_FLOOR_RIGHT1:uint	= 6;
		public static const DBLSLOPE_FLOOR_RIGHT2:uint	= 7;
		public static const SLOPE_CEIL_LEFT:uint   		= 8;
		public static const SLOPE_CEIL_RIGHT:uint  		= 9;
		...
		override public function loadMap(MapData:String, TileGraphic:Class, TileWidth:uint=0, TileHeight:uint=0, AutoTile:uint=OFF, StartingIndex:uint=0, DrawIndex:uint=1, CollideIndex:uint=1):FlxTilemap
		{
			...
			setTileProperties(DBLSLOPE_FLOOR_LEFT1, FLOOR, solveCollisionDoubleSlopeFloorLeft_1);
			setTileProperties(DBLSLOPE_FLOOR_LEFT2, FLOOR, solveCollisionDoubleSlopeFloorLeft_2);
			setTileProperties(DBLSLOPE_FLOOR_RIGHT1, FLOOR, solveCollisionDoubleSlopeFloorRight_1);
			setTileProperties(DBLSLOPE_FLOOR_RIGHT2, FLOOR, solveCollisionDoubleSlopeFloorRight_2);
			return tmpTileMap;
		}
		 ...
		 * @param	Position			Optional, specify a custom position for the tilemap (useful for overlapsAt()-type funcitonality)._slopePoint.y = (slope.y + _tileHeight) - ((_slopePoint.x - slope.x) * Math.tan(22.5 * 0.017453293));
		 * 
			...
			while(row < selectionHeight)
			{
				...
				while(column < selectionWidth)
				{
					...
					if(tile.allowCollisions)
					{
						...
						if (overlapFound 
						|| (!overlapFound && (tile.index == SLOPE_FLOOR_LEFT || DBLSLOPE_FLOOR_LEFT1 || DBLSLOPE_FLOOR_LEFT2 || DBLSLOPE_FLOOR_RIGHT1 || DBLSLOPE_FLOOR_RIGHT2 || tile.index == SLOPE_FLOOR_RIGHT || tile.index == SLOPE_CEIL_LEFT || tile.index == SLOPE_CEIL_RIGHT)))
						{
		...
		final private function solveCollisionDoubleSlopeFloorLeft_1(slope:FlxTile, obj:FlxObject):void
		{						
			//calculate the corner point of the object
			_objPoint.x = FlxU.floor(obj.x + obj.width + _snapping);
			_objPoint.y = FlxU.floor(obj.y + obj.height);
			...
			//this would be one side of the object projected onto the slope's surface
			_slopePoint.x = _objPoint.x;
			_slopePoint.y = (slope.y + _tileHeight/2) - ((_slopePoint.x - slope.x) * Math.tan(22.5 * 0.017453293));
		...
		final private function solveCollisionDoubleSlopeFloorLeft_2(slope:FlxTile, obj:FlxObject):void
		{						
			//calculate the corner point of the object
			_objPoint.x = FlxU.floor(obj.x + obj.width + _snapping);
			_objPoint.y = FlxU.floor(obj.y + obj.height);
			...
			//this would be one side of the object projected onto the slope's surface
			_slopePoint.x = _objPoint.x;
			_slopePoint.y = (slope.y + _tileHeight) - ((_slopePoint.x - slope.x) * Math.tan(22.5 * 0.017453293));
		...
		final private function solveCollisionDoubleSlopeFloorRight_1(slope:FlxTile, obj:FlxObject):void
		{						
			//calculate the corner point of the object
			_objPoint.x = FlxU.floor(obj.x + obj.width + _snapping);
			_objPoint.y = FlxU.floor(obj.y + obj.height);
			...
			//this would be one side of the object projected onto the slope's surface
			_slopePoint.x = _objPoint.x;
			_slopePoint.y = (slope.y + _tileHeight/2) - ((slope.x - _slopePoint.x + _tileWidth) * Math.tan(22.5 * 0.017453293));
		...
		final private function solveCollisionDoubleSlopeFloorRight_2(slope:FlxTile, obj:FlxObject):void
		{						
			//calculate the corner point of the object
			_objPoint.x = FlxU.floor(obj.x + obj.width + _snapping);
			_objPoint.y = FlxU.floor(obj.y + obj.height);
		...
		{							
			...
			//this would be one side of the object projected onto the slope's surface
			_slopePoint.x = _objPoint.x;
			_slopePoint.y = (slope.y + _tileHeight) - ((slope.x - _slopePoint.x + _tileWidth) * Math.tan(22.5 * 0.017453293));
